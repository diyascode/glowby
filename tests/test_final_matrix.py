...
# 113. A CHECK IS A CHECK (Sept 22, Inderpreet: "17 checks but three in
# recent checks"): only a submitted check adds to the day's tally; the
# follow-up answer, the regenerated one-line, the scam lens, reviews and
# calibration add spend only.
_src113 = open("app/main.py").read()
_calls113 = [l for l in _src113.split("\n") if "add_usage(" in l and "def " not in l and "import" not in l and l.strip() != "add_usage,"]
_counted113 = [l for l in _calls113 if "count=False" not in l]
if len(_counted113) != 1 or "COST_PER_CHECK_EST" not in _counted113[0]: fails.append(f"m113 counted call sites: {_counted113}")
